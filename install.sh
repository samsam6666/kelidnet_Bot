...
setup_env_file() {
    ...
    if [ -f ".env" ]; then
        read -p "$(echo -e ${YELLOW}"File .env already exists. Overwrite? (y/n): "${NC})" overwrite
        if [[ "$overwrite" != "y" ]]; then print_success "Skipping .env configuration."; return; fi
    fi

    # --- بخش اصلاح شده ---
    # دریافت هر سه متغیر ضروری
    read -p "$(echo -e ${YELLOW}"Please enter your Telegram Bot Token from BotFather: "${NC})" bot_token
    read -p "$(echo -e ${YELLOW}"Please enter your numeric Admin ID: "${NC})" admin_id
    read -p "$(echo -e ${YELLOW}"Please enter your bot's username (without @, e.g., AlamorVPNBot): "${NC})" bot_username
    # --- پایان بخش اصلاح شده ---
